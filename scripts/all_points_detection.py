...
def detect_points(data: np.ndarray, tol: int) -> tuple:
    ...
    for i in range(1, x_data.size):
        ...
        if (
            dx_sign != prev_sign and x_act <= tol
        ):  # here is identified the LAST spike (at the end of the Burst Duration)
            if spike_marked:
            ...
            firsts_location.append(
                i - 1
            )  # here is identified the FIRST spike (at the begining of the Burst Duration)
...
if __name__ == "__main__":
    ...
    (firsts_location, lasts_location) = detect_points(
        np.array([df["x"].to_list(), df["time"].to_list()]), -1.2
    )
    ...
    plt.plot(
        [list_t[i] for i in firsts_location],
        [list_x[i] for i in firsts_location],
        marker=".",
        ls="none",
        ms=10,
        color="r",
    ...
    plt.plot(
        [list_t[i] for i in lasts_location],
        [list_x[i] for i in lasts_location],
        marker=".",
        ls="none",
        ms=10,
        color="g",
    ...
    if df_firsts["time"].values[0] > df_lasts["time"].values[0]:
        ...
        for i in range(
            max(len(df_firsts["first"].values), len(df_lasts["last"].values))
        ):
    ...
    df_periods = df_analyzed_values.drop(
        ["interburst_duration", "burst_duration", "time_interburst", "time_burst"],
        axis=1,
    ).dropna()
    df_interbursts = df_analyzed_values.drop(
        ["period_duration", "burst_duration", "time_period", "time_burst"], axis=1
    ).dropna()
    df_bursts = df_analyzed_values.drop(
        ["period_duration", "interburst_duration", "time_period", "time_interburst"],
        axis=1,
    ).dropna()

    periods_list = [[time, period + time] for time, period in df_periods.values]
    interbursts_list = [
        [time, interburst + time] for time, interburst in df_interbursts.values
    ]
    ...
    plt.legend(
        loc="lower left",
        handles=[period_lines[0], burst_lines[0], interburst_lines[0]],
        labels=["period", "burst duration", "interburst interval"],
    )
    ...
    df_interbursts["interburst_duration"] = df_interbursts["interburst_duration"].apply(
        lambda x: int(x)
    )
    axs[0].hist(df_bursts["burst_duration"], bins=20)
